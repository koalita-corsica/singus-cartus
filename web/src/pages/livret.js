...
const LivretAPI = (props) => {
    ...
    const getDataCompany = async () => {
        ...
        .then(function (response) {
            ...
            setAdresse(response.data.data.adresse.rue + " " + response.data.data.adresse.code_postal)
            setTelephone(response.data.data.contact.telephone)
            setMail(response.data.data.contact.email)
            setGerant(response.data.data.gerants[0].prenom + " " + response.data.data.gerants[0].nom)
            setActivite(response.data.data.activite.activite)
        })
    ...
    function handleSubmit1() {
        ...
        client.create(doc).then((res) => {
            ...
            logo.toBlob(uploadImageBlob, 'image/png')
